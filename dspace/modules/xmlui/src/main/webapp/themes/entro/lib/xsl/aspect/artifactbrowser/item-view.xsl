    ...
    <xsl:template name="itemSummaryView-DIM-fields">
      ...
      <xsl:choose>
          ...
          <xsl:when test="$clause = 4 and (dim:field[@element='date' and @qualifier='issued' and descendant::text()])">
            ...
            <xsl:when test="$clause = 6 and (dim:field[@element='theme' and @qualifier='nbi' and descendant::text()])">
                    <div class="simple-item-view-other">
	                <span class="bold"><i18n:text>Themes</i18n:text>:</span>
	                <span>
	                	<xsl:for-each select="dim:field[@element='theme' and @qualifier='nbi']">
		                     <xsl:copy-of select="./node()"/>
                            <xsl:if test="count(following-sibling::dim:field[@element='theme' and @qualifier='nbi']) != 0">
		                    	<br/>
		                    </xsl:if>
	                    </xsl:for-each>
	                </span>
